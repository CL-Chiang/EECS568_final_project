...
public:
    mapOptimization()
    {
        ...
        // savePCDDirectory = std::getenv("HOME") + savePCDDirectory; // rather use global path 
          int unused = system((std::string("exec rm -rf ") + savePCDDirectory).c_str());
          unused = system((std::string("mkdir ") + savePCDDirectory).c_str());

          saveSCDDirectory = savePCDDirectory + "SCDs/"; // SCD: scan context descriptor 
          unused = system((std::string("exec rm -rf ") + saveSCDDirectory).c_str());
          unused = system((std::string("mkdir -p ") + saveSCDDirectory).c_str());

          saveNodePCDDirectory = savePCDDirectory + "Scans/";
          unused = system((std::string("exec rm -rf ") + saveNodePCDDirectory).c_str());
	...
	//file.open("/home/kechen/timestampsinUpdate.txt",std::ios_base::app);
	//file << pose_stamped.header.stamp<<endl;
	//file.close();
    }
	...
	//timestamps.push_back(laserOdometryROS.header.stamp.toSec());

        //timestamps.append('\n')
	// file.open("/home/kechen/timestamps.txt",std::ios_base::app);
       // file << laserOdometryROS.header.stamp<<endl;
       // file.close();
        // Publish TF
...
int main(int argc, char** argv)
{
    ...
    cout<<"1"<<std::endl;
    //file << MO.timestamps<<endl;
    
    ros::spin();
    //ofstream file;
   // file.open("/home/kechen/timestampsinUpdate.txt",std::ios_base::app);
    //for(int i=0;i<MO.timestamps.size(); i++){
//	file <<MO.timestamps[i]<<std::endl;}
  //  file.close();
